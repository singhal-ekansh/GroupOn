class Deal < ApplicationRecord
  ...
  with_options if: -> { published } do |deal|
    ...
    deal.validates :images, :length => { :minimum => 1, message: 'must include a image' }
  ...
  accepts_nested_attributes_for :images, allow_destroy: true, reject_if: ->(attr) { !attr.key?('file') }
